...
import { FaChartLine, FaList, FaUsers } from "react-icons/fa";
    ...
    <div>
      <div className="p-8 bg-gray-100">
        <div className="grid grid-cols-4 gap-4">
          <StatCard value={Helper.formatNumber(analysis.countLoaner, 0)} title={"จำนวนผู้กู้"} icon={<FaUsers />} color={"bg-purple-500"}></StatCard>
      ...
      <div className="px-8 py-2">
        <select placeholder="เลือกหมู่บ้าน" className="w-full bg-gray-50 transition-all hover:bg-gray-100 px-4 py-2 rounded-xl outline-none" value={villageSelect} onChange={(e) => gotoVillageRoute(e.target.value)}>
      ...
      <div className="px-8 py-4 flex gap-4 items-center">
        <Input variant="bordered" type="date" className="w-full" label="วันที่เริ่มต้น" value={startDate} onChange={(e) => gotoStartDateRoute(e.target.value)} />
        <Input variant="bordered" type="date" className="w-full" label="วันที่สิ้นสุด" value={endDate} onChange={(e) => gotoEndDateRoute(e.target.value)} />
