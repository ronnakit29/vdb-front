...
export default function PromiseFinal() {
    ...
    function handleUpdateCitizenData() {
        dispatch(showConfirm("ยืนยันการอัพเดทข้อมูลบัตรประชาชน", () => {
            console.log(insertSlot)
            dispatch(updatePromiseDocument(groupId, {
    ...
    function handleEndPromise() {
        dispatch(showConfirm('ยืนยันการสิ้นสุดสัญญานี้', () => {
    ...
    return (
        <div>
            ...
            {memberDocs.length > 0 && <div className='p-8'>
                <div className="text-xl text-default-900 font-semibold mb-3">รายละเอียด <span className="px-3 py-1 text-sm rounded-full bg-green-500 text-white">{groupId}</span></div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    {memberDocs.map((i, key) => <div className='mb-4 p-4 rounded-3xl bg-gray-100'>
                        <h3 className='text-lg font-semibold text-green-500 mb-2'>สัญญาที่ {key + 1}/{memberDocs.length}</h3>
                        <Divider />
                ...
                <SectionForm title={"ข้อมูลผู้จัดการ/พนักงาน"}>
                    ...
                    {(memberDocs[0].manager_citizen_id && memberDocs[0].employee_citizen_id) && <div>
                        - ได้รับข้อมูลของผู้จัดการและพนักงานแล้ว -
                    </div>}
                </SectionForm>
