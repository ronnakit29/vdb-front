...
export default function Index() {
  ...
  async function checkManagerAndSave(citizen_id) {
    try {
      dispatch(
        createIncomeExpenses({ ...formData, citizen_id }, () => {
          init();
          resetForm();
        })
      );
  ...
  function init() {
    if (villageSelect) dispatch(getIncomeExpenseList(villageSelect || user?.village?.id, startDate, endDate));
  ...
  return (
    <div>
      ...
      <div className="px-8 py-2">
        <select placeholder="เลือกหมู่บ้าน" className="w-full bg-gray-50 transition-all hover:bg-gray-100 px-4 py-2 rounded-xl outline-none" value={villageSelect} onChange={(e) => setVillageSelect(e.target.value)}>
      ...
      <div className="px-8 py-2 flex justify-between gap-4 items-center">
        ...
        <Button color="success" className="text-white gap-2 items-center" onClick={() => router.push("/dashboard/income-expense/create")}>
          <FaPlus />
          สร้างรายการใหม่
        </Button>
      </div>
